...
function render_stars($comment_field)
{
    ...
    if (!is_singular('recipes') && !is_product() && !is_singular('meal-plans') && !is_singular('multiple-recipes') ) {
...
function add_rating_to_review_text( $text ) {
	...
	if ( is_admin() || (!is_singular('recipes') && !is_product() && !is_singular('meal-plans') && !is_singular('multiple-recipes') ) ) {
		return $text;
	}
