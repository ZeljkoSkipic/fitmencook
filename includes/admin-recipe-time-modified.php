...
function admin_scripts($hook)
{
    if ($hook === 'post.php') {
        ...
        if ($post_ID) {
            ...
            if ($post_type === 'recipes') {
                wp_enqueue_script('recipes-admin-js', get_template_directory_uri() . '/js/admin.min.js');
                wp_enqueue_style('admin-recipe-time-modified', get_template_directory_uri() . '/admin-css/admin.css');
            }
        }
    }
    ...
    wp_localize_script('recipes-admin-js', 'globalAdminObject', $localize_object);
}
...
add_filter('manage_recipes_posts_columns', 'add_custom_columns_recipes');
...
add_action('manage_recipes_posts_custom_column', 'custom_columns_content', 10, 2);
...
add_action('post_submitbox_misc_actions', 'change_recipe_modify_time_form', 1);

function change_recipe_modify_time_form($post)
{
